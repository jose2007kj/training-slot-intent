...
#			y = self.model.layers[9].output
	...
	def build( self ):
		...
		if self.arch == 'lstm' or self.arch == 'rnn' or self.arch == 'gru' or self.arch == 'blstm' or self.arch == 'brnn' or self.arch == 'bgru':
			...
			intent_p = TimeDistributed(Dense(self.intent_vocab_size, activation='softmax'))(tagger)
			self.model = Model(input=raw_current, output=[prediction,intent_p])
		...
		# Encode all history and the current utterance first and then start tagging
			...
			if 'blstm' in self.arch:
				...
				backward = LSTM(self.hidden_size, return_sequences=True, init=self.init_type, activation=self.activation, go_backwards=True)(current)
				labeling = merge([forward, backward], mode='concat', concat_axis=-1)
				intent = merge([forward, backward], mode='concat', concat_axis=-1)
			elif 'rnn' in self.arch:
			...
			tagger = merge([encoder, labeling], mode='concat', concat_axis=-1)
			intent_p = merge([encoder, intent], mode='concat', concat_axis=-1)
			if self.dropout:
				tagger = Dropout(self.dropout_ratio)(tagger)
				intent_p = Dropout(self.dropout_ratio)(intent_p)

			prediction = TimeDistributed(Dense(self.output_vocab_size, activation='softmax'))(tagger)
			intent_prediction = TimeDistributed(Dense(self.intent_vocab_size, activation='softmax'))(intent_p)
			self.model = Model(input=[raw_input_memory, raw_current], output=[prediction,intent_prediction])
			self.model.compile(loss='categorical_crossentropy', optimizer=opt_func)


	def train(self, H_train, X_train, y_train, y1_train, H_dev, X_dev, y_dev, val_ratio=0.0):
		...
		if self.load_weight is not None:
			...
			if not self.nodev:
				...
				self.model.fit(batch_train,[y_train,y1_train], batch_size=self.batch_size, nb_epoch=self.max_epochs, verbose=1, validation_data=(batch_dev, y_dev), callbacks=[early_stop, train_log], shuffle=self.shuffle)
				...
				if self.set_batch:
					...
					self.model.fit(batch_train,[y_train,y1_train], batch_size=self.batch_size, nb_epoch=self.max_epochs, verbose=1, shuffle=self.shuffle)
					print("model.fit=====2")
					self.model.save('train.h5')
	def run(self):
		...
		emptyVocab = {}
		emptyIndex = list()
		trainData = dataSet(self.training_file,'train',emptyVocab,emptyVocab,emptyVocab,emptyIndex,emptyIndex,emptyIndex)
		testData = dataSet(self.test_file, 'test', trainData.getWordVocab(), trainData.getTagVocab(),trainData.getIntentVocab(),trainData.getIndex2Word(),trainData.getIndex2Tag(),trainData.getIndex2Intent())
		...
		intent_file = "intent.list"
		fo = open(target_file, "wb")
		for tag in trainData.dataSet['id2tag']:
			print('tags:',tag)
			fo.write("%s\n" %tag)
		fo.close()
		fo = open(intent_file, "wb")
		for intent in trainData.dataSet['id2intent']:
			print('tags:',intent)
			fo.write("%s\n" %intent)
		fo.close()
		...
		pad_y1_train = sequence.pad_sequences(trainData.dataSet['intents'], maxlen=10, dtype='int32', padding='pre')
		...
		self.intent_vocab_size = trainData.getIntentVocabSize()
		...
		y1_train = encoding(pad_y1_train, '1hot', self.time_length, self.intent_vocab_size)
