...
class dataSet(object):
   ...
   def __init__(self,dataFile,toggle,wordDictionary,tagDictionary,intentDictionary,id2word,id2tag,id2intent):
       if toggle == 'train':
         self.dataSet = readData(dataFile)
       if toggle == 'val':
         self.dataSet = readTest(dataFile,wordDictionary,tagDictionary,intentDictionary,id2word,id2tag,id2intent)
       if toggle == 'test':
         self.dataSet = readTest(dataFile,wordDictionary,tagDictionary,intentDictionary,id2word,id2tag,id2intent)
   ...
   def getIndex2Intent(self):
   		return self.dataSet['id2intent']	
...
# if the input is multiturn session data, the flag following the IOB tags is 1 (session start) or 0 (not session start)
	...
	word_vocab_index = 2
	tag_vocab_index = 2
	intent_vocab_index = 2
	word2id = {'<pad>': 0, '<unk>': 1}
	tag2id = {'<pad>': 0, '<unk>': 1}
	intent2id = {'<pad>': 0, '<unk>': 1}
	id2word = ['<pad>', '<unk>']
	id2tag = ['<pad>', '<unk>']
	id2intent = ['<pad>', '<unk>']
	...
	for line in open(dataFile, 'r'):
		...
		utterances.append(temp_utt)
		tags.append(temp_tags)
		for i in xrange(len(myintents)):
			if myintents[i] not in intent2id:
				intent2id[myintents[i]] = intent_vocab_index
				id2intent.append(myintents[i])
				intent_vocab_index += 1
			temp_intents.append(intent2id[myintents[i]])
		intents.append(temp_intents)
	data = {'start': starts, 'startid': startid, 'utterances': utterances, 'tags': tags,'intents': intents, 'uttCount': utt_count, 'id2word':id2word, 'id2tag':id2tag, 'id2intent': id2intent, 'wordVocabSize' : word_vocab_index, 'tagVocabSize': tag_vocab_index, 'intentVocabSize' : intent_vocab_index , 'word2id': word2id, 'tag2id':tag2id, 'intent2id': intent2id}
	return data

def readTest(testFile,word2id,tag2id,intent2id,id2word,id2tag,id2intent):
	...
	#print 'utt: %s, tags: %s' % (utt,t) 
		...
		for i in xrange(len(myintents)):
			if myintents[i] not in intent2id:
				temp_intents.append(1)
			else:
				temp_intents.append(intent2id[myintents[i]])
		intents.append(temp_intents)
		#---------
		wordVocabSize = len(word2id)
		intentVocabSize= len(intent2id)

	data = {'start': starts, 'startid': startid, 'utterances': utterances, 'tags': tags,'intents': intents, 'uttCount': utt_count, 'wordVocabSize' : wordVocabSize, 'intentVocabSize' : intentVocabSize , 'id2word':id2word, 'id2tag': id2tag, 'id2intent': id2intent}
	return data
